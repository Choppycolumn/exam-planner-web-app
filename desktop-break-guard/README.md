# Break Guard

Windows 学习计时与休息提醒程序。运行状态、课程记录、待同步事件和日志保存在 `%LOCALAPPDATA%\ExamPlanner\BreakGuard`，网站令牌使用 Windows DPAPI 加密。

## 日常使用

- 课程直接来自网站学习项目，不再设置固定节数；点击科目后开始记录实际学习时长。
- 专注小窗支持课程内分段计时；已结束分段会跨程序重启保留，并在本次课程总计时结束时清空。
- 在“学习设置”中设置每日目标时长、课间休息、进度提醒宽限和超长计时核对阈值；超过阈值后，结束课程前必须确认实际分钟数。
- “学习设置 → 学习记录”可按日期查看已完成课程，直接修改分钟数或删除误记；本地立即生效，网站按差额自动同步，断网时保留在 outbox 中等待补传。
- 休息结束后仍保留原有全屏提醒、1 分钟网站通知和 5 分钟不专注记录。
- 当每日已学时长未达目标且休息超过宽限时间时，程序会全屏提醒，并通过网站通知系统发送进度提醒。
- 午饭和晚饭为不计时暂停；开始下一次学习后自动恢复进度监测。
- 点击“结束一天”会保存当天结束状态并展示总时长、完成课次、目标完成度和各科投入；当天的进度催促会暂停到次日。忘记点击时，程序会在跨日后自动收口昨天并关闭旧提醒。需要继续时，可在总结中重新开启今天。

## 开发运行

```powershell
python -m pip install -r requirements.txt
python break_guard.py
```

## 测试与打包

```powershell
python -m unittest discover -s tests -v
.\build.ps1
```

打包结果为 `dist\BreakGuard.exe`。程序使用 Windows Named Mutex 保证单实例，第二次启动只会显示已有窗口。

`dist\build-manifest.json` 记录版本、构建时间、SHA-256 和签名状态。`install_startup.ps1` 优先安装登录计划任务，异常退出后最多自动重启 10 次；权限不足时退化为启动文件夹快捷方式。

托盘在 Explorer 尚未就绪或重启后会指数退避重连；启动链会等待真实应用进程，使计划任务能够识别异常退出。

桌面和开机启动快捷方式统一调用无控制台启动器。系统允许本地未签名程序时优先运行打包版；Windows Smart App Control 拦截未签名 EXE 时自动改用已签名的 `pythonw.exe` 运行同一份源码，不关闭或绕过系统安全策略。
