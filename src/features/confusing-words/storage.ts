...
const storageKey = (userId: number) => `examPlanner.confusingWords.v1.user-${userId}`;
...
export function loadGroups(userId: number, { allowLegacy = false } = {}): ConfusingWordGroup[] {
  const raw = localStorage.getItem(storageKey(userId)) ?? (allowLegacy ? localStorage.getItem('examPlanner.confusingWords.v1') : null);
  if (!raw) {
    const examples = [
...
export function saveGroups(groups: ConfusingWordGroup[], userId: number) {
...
export function loadConfusingWordsExport(userId: number, { allowLegacy = false } = {}): ConfusingWordsExport | null {
  const raw = localStorage.getItem(storageKey(userId)) ?? (allowLegacy ? localStorage.getItem('examPlanner.confusingWords.v1') : null);
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as ConfusingWordsExport;
