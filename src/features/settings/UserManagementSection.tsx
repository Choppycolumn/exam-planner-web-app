    ...
    <section className="mt-5 card p-5" aria-labelledby="user-management-title">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-secondary">每个用户拥有独立课程、计时、复盘和单词数据；学习对比仅共享汇总时长。</p>
        </div>
        <span className="app-status-pill">{data ? `${data.userCount} / ${data.maxUsers} 个席位` : '读取中'}</span>
      </div>

      <div className="mt-4 divide-y divide-slate-100 overflow-hidden rounded-lg border border-line">
        {data?.users.map((account) => (
          <div key={account.userId} className="grid gap-3 px-4 py-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-secondary">创建于 {new Date(account.createdAt).toLocaleString()} · 最近登录 {account.lastLoginAt ? new Date(account.lastLoginAt).toLocaleString() : '暂无'}</p>
            </div>
      ...
      <div className="mt-5 border-t border-line pt-5">
        <h3 className="text-sm font-semibold text-primary">邀请新用户</h3>
        ...
        {latestInvite ? <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-accent bg-accent-soft p-3 text-sm text-accent"><code className="min-w-0 flex-1 break-all">{latestInvite.token}</code><button className="btn btn-soft" type="button" onClick={() => void copyInvite()}><Copy size={15} />复制</button><span className="text-xs">有效至 {new Date(latestInvite.expiresAt).toLocaleString()}</span></div> : null}
      </div>

      {data?.invites.some((invite) => !invite.usedAt && !invite.revokedAt) ? <div className="mt-4 text-xs text-secondary">当前有效邀请码：{data.invites.filter((invite) => !invite.usedAt && !invite.revokedAt).length} 个。邀请码只在创建时完整显示，可随时撤销并重新生成。</div> : null}
    </section>
  );
}
