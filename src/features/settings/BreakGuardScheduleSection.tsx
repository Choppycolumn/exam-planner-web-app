    ...
    <section className="mt-5 card p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">桌面端直接展示下方学习项目，不再安排固定节数。结束学习后，实际分钟会累计到所选项目。</p>
        </div>
      ...
      <div className="mt-5">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {data.projects.map((project) => (
            <span key={project.id} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700">
              <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: project.color }} />
              {project.name}
            </span>
          ))}
          {!data.projects.length && <span className="text-sm text-slate-500">暂无学习项目，请先在学习时间页面添加。</span>}
        </div>
      </div>
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-slate-500">进度条按“今日已学时长 / {Math.floor(data.config.dailyTargetMinutes / 60)} 小时 {data.config.dailyTargetMinutes % 60} 分钟”计算。</p>
