...
function encryptSettingSecret(value = '') {
    ...
    const cipher = runtime.createCipheriv('aes-256-gcm', runtime.settingsEncryptionKey, iv);
...
function decryptSettingSecret(value = '') {
    ...
    try {
        ...
        const decipher = runtime.createDecipheriv('aes-256-gcm', runtime.settingsEncryptionKey, Buffer.from(iv, 'base64url'));
        decipher.setAuthTag(Buffer.from(tag, 'base64url'));
        return Buffer.concat([decipher.update(Buffer.from(encrypted, 'base64url')), decipher.final()]).toString('utf8');
    }
    catch (error) {
        runtime.logStructured('warn', 'settings_secret_decrypt_failed', { error: runtime.redactSecretText(error.message || String(error)) });
        return '';
    }
}
